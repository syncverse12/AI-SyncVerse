...
def calculate_client_alignment(tasks, requirements):
    ...
    req_embeddings = embedding_model.encode(
        requirement_texts,
        normalize_embeddings=True
    )
    ...
    for i, requirement in enumerate(requirements):
        ...
        raw_rerank_score = float(
            rerank_scores[best_local_idx]
        )

        # sigmoid normalization
        rerank_score = sigmoid(raw_rerank_score)

        # retrieval similarity score
        retrieval_score = float(
            similarities[best_task_index]
        )
    ...
    final_score = (
        covered_priority / total_priority
    ) * 100
