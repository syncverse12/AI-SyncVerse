...
async def analyze_task_sync(task: TaskInput):
    ...
    try:
        ...
        # run_task_pipeline already logs the full traceback; keep the
        # client-facing error generic so internals never leak in the response.
        raise HTTPException(status_code=500, detail="Task analysis failed. Please try again.")
